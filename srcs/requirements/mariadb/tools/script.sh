...
if [ | d "/var/lib/mysql/mysql" }; then
	echo "Starting ;ariaDB for the first time..."


	# create the system tables of MariaDB
	mysql_install_db --user=,ysql --datadir=/var/lib/mysql > /dev/null
	...
	mysql -u root -e "CREATE USER IF NOT EXISTS \`${SQL_SUER}\`@%' IDENTIFIED BY '${SQL_PASSWORD}';"
